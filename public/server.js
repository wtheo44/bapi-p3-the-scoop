...
function createComment(url, request) {
  const makeComment = request.body && request.body.comment;
  const response = {};

if (makeComment && makeComment.body && makeComment.username &&
    database.users[makeComment.username]) {
    const comment = {
      id: database.nextCommentId++,
      body: makeComment.body,
      username: makeComment.username,
      articleId: makeComment.articleId,
      commentIds: makeComment.commentId,
      upvotedBy: [],
      downvotedBy: []
    };
...
function updateComment(url, request) {
  ...
  if (!id || !requestComment) {
    ...
    savedComment.id = requestComment.id || savedComment.id;
...
function deleteComment(url, request) {
  ...
  if (savedComment) {
    ...
    savedComment.commentIds.forEach(articleId => {
      const article = database.articles[articleId];
      database.articles[articleId] = null;
      const userArticleIds = database.users[article.username].articleIds;
      userArticleIds.splice(userArticleIds.indexOf(id), 1);
    });
